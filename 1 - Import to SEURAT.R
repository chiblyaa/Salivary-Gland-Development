...
DimPlot(e16smg)
e16smg<-RenameIdents(e16smg, 'Glial cells' = "Nerves", "Krt19+ duct" ="Duct", "Basal duct" = "Duct", "Smooth muscle" = "Mesenchyme")
Idents(e16smg) <- factor(Idents(e16smg), levels = sort(levels(Idents(e16smg)),decreasing = F))
e16smg[["TestID"]] <- Idents(e16smg)
e16smg <- SetIdent(e16smg, value = "TestID")
e16smg.sub <- subset(e16smg,idents = "Erythroid", invert=TRUE)
DimPlot(e16smg.sub)

AvgExpression.allclusters <- AverageExpression(e16smg.sub, assays = "RNA")
...
Receptors <- merge(allaverages, Ligand.Receptor.Pairs, by.x = "gene", by.y = "Receptor")
Ligands <- merge(allaverages, Ligand.Receptor.Pairs, by.x = "gene", by.y = "Ligand")
##### Add ligand expression info to receptor table to identify all potential ligand-receptor pairs
potential.Pairs <- merge(Receptors, allaverages, by.x = "Ligand", by.y = "gene", no.dups = F)
names(potential.Pairs)[2] <- "Receptor"
write.table(potential.Pairs, file = "E16 All potential Ligand-Receptor Pairs.txt", sep = "\t", row.names =F)
##### create matrix with number of potential pairs
counter = c(0,0,0,0,0,0,0)  #create counter vector with 10 slots for 10 cell types
number.of.pairs <- matrix(ncol = 7, nrow = 7, dimnames = list(colnames(potential.Pairs[3:9]), colnames(potential.Pairs[3:9]) )) # create 10X10 matrix to save number of pairs between cell types
for (j in 1:7){
  counter = c(0,0,0,0,0,0,0)
  for (i in 1:NROW(potential.Pairs)){
    if(is.na(potential.Pairs[i,j+2])==0){
      for(k in 1:7){
        if(is.na(potential.Pairs[i,k+12])==0){
          counter[k] <- counter[k] + 1
        }
      }
    }
  }
  number.of.pairs[j,] <- counter[1:7]
}
#### The transposed matrix 'Number of Pairs 2' is organized to plot the recipient (receptor) cells in columns and provider (ligands) cells in rows
#### The resulting plot is ligands from MECs towards receptors in all other cells
number.of.pairs <- t(number.of.pairs)
#### chord diagrams
border_mat2 = matrix("black", nrow = 1, ncol = ncol(number.of.pairs))
rownames(border_mat2) = rownames(number.of.pairs)[8]
colnames(border_mat2) = colnames(number.of.pairs)
grid.col = c("Myoepithelial.x" = "#990000", 
             'End bud.x' = "#0066CC", 
             'Duct.x' = "#00CC66", 
             "Mesenchyme" = "#FF00FF",
             "Endothelial.x" = "#994C00", 
             'Immune.x'= "#FF99FF", 
             'Nerves.x' = "#00FFFF")
circos.par(gap.degree=5, gap.after=5)
chordDiagram(number.of.pairs, grid.col = grid.col, link.lwd = 1, link.lty = 1, link.border = border_mat2, 
             ...
             annotationTrack = "grid", link.largest.ontop = T, row.col = c("#0066CC33", "#FF00FF33", "#00CC6633",   "#00FFFF33", "#D2B48C33", "#FFD70033", "#FF99FF33", "#990000", "#994C0033", "#80008033") )
